...
bool g_bExit = false;
Mat img(ROI_HEIGHT, ROI_WIDTH, CV_8U, Scalar(0));
...
static void* localize3D(void* pImg)
{
    QRLocation QRR;
    while(1)
	{
	    QRR.callLocalizationFunction(pImg);
	}
...
int main()
{
	...
	*/
	...
	/*sleep(1);
	pthread_t nThreadIDLoc; 
	Mat* pImg = &img;
	int nRetLoc = pthread_create(&nThreadIDLoc, NULL ,localizeXYZ, (void*)pImg);
	
	if (nRetLoc != 0)
	{
		printf("threadLoc create failed.ret = %d\n",nRet);
		return -1;
	}*/

	sleep(1);
	
	pthread_t nThreadIDLocResize; 
	Mat* pImgResize = &img;
	int nRetLocResize = pthread_create(&nThreadIDLocResize, NULL ,localize3D, (void*)pImgResize);
