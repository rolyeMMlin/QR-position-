...
// Function prototypes
static void doBasicDemo(const char* text, int edgeLength, const char* picName);
...
int main(int argc,   char* argv[]) {
	...
	const char* picName = argv[3];
	int edgeLength = atoi(edgeLengthChar);
	doBasicDemo(text, edgeLength, picName);
	return EXIT_SUCCESS;
}
...
void getOutputImgWithArrow(Mat& imgOutput, Mat QRimg){
	...
	rectangle(imgOutput, Rect(0, 0, imgOutput.cols, imgOutput.rows), Scalar(0), 1);
}
