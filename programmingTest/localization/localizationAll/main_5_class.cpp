...
int main(){
	...
	while (!kbhit()){
		...
		case 65:   // 0x41
			...
			y = postionInfoValues[1];
			postionInfoValues[1] =  postionInfoValues[1] - leftCamera.yDiff;
			d_2 = postionInfoValues[6]*cos(abs(postionInfoValues[7]/1800.0*3.14));
			d_3 = sqrt(d_2*d_2 - y*y);
			postionInfoValues[1] += int((-(d_3+270)*tan(postionInfoValues[2]/1800.0*3.14))+0.5);
