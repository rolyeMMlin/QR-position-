...
bool getQRPoistionInfo(int postionInfo[], void* img, float resizePara, bool isCalibrate, \
						float codeLength, float upCenterX, float lowerCenterY, bool isUp, int codeNumbers, int codeInfo[]){
	QRLocation QRR;
	...
	if (isLocalization){
		...
		*(++postionInfo) = QRR.isError;
		...
		if (QRR.codeInfo.size()==2)
		{
			if (strcmp(QRR.codeInfo[0].c_str(), QRR.codeInfo[1].c_str()))
			{
				cout<< "!!!!!!!!!!!!ERROR!!!!!!!!!!!!!!!!!"<<endl;
				return false;
			}
			codeInfo[0] = atoi(QRR.codeInfo[0].c_str());	
		}
		else{
			if (strcmp(QRR.codeInfo[0].c_str(), QRR.codeInfo[2].c_str()))
				QRR.codeInfo.erase(QRR.codeInfo.begin()+1);
			else
				QRR.codeInfo.erase(QRR.codeInfo.begin());
			
			//cout<<"~~~~~~~~"<<QRR.codeInfo[0]<<endl;
			//cout<<"~~~~~~~~"<<QRR.codeInfo[1]<<endl;
			codeInfo[0] = atoi(QRR.codeInfo[0].c_str());
			codeInfo[1] = atoi(QRR.codeInfo[1].c_str());
		}
		
		//cout<< "time of string process is: "<< (clock()-t1)/1000.0<<"ms" << endl;*/
	}
...
vector<Point> QRLocation::zbarScan(Mat& input, int centerPos[])//二维码扫描接口
{
...
bool QRLocation::callLocalizationByRmatrix(void* pImg, float resizePara, \
	...
	double t1 = clock();
	vector<Point> AllQR = zbarScan(img, centerPos_first);
	...
	vector<Point> AllQRRegion = zbarScan(QRRegion, centerPos);
